...
static struct kmem_cache *br_igmp_mc_fil_cache __read_mostly;
...
int br_igmp_control_filter(const unsigned char *dest, __be32 dest_ip)
{
	...
	// Broadcast and 224.0.0.x are always blocked
	...
	hlist_for_each_entry(filter_entry, h_group, &multiConfig.igmp_control_filter_list, node)
	{
...
void br_igmp_wipe_ignore_group_list ( void )
{
	struct br_mcast_control_filter_entry *filter_entry = NULL;
	struct hlist_node *h_group, *n_group;

	hlist_for_each_entry_safe(filter_entry, h_group, n_group, &multiConfig.igmp_control_filter_list, node)
	{
		hlist_del(&filter_entry->node);
		kmem_cache_free(br_igmp_mc_fil_cache, filter_entry);
	}
}

int br_igmp_process_ignore_group_list (int count, t_MCPD_IGNORE_GROUP_ENTRY* ignoreMsgPtr)
{
	int inputIndex = 0;
	int ret = 0;
	
	spin_lock_bh(&multiConfig.cfgLock);
	br_igmp_wipe_ignore_group_list();

	for ( ; inputIndex < count; inputIndex ++)
	{
		if ((ignoreMsgPtr[inputIndex].mask.s6_addr32[0] & htonl(0xF0000000)) != htonl(0xF0000000) ) 
		{
			ret = -EINVAL;
			printk ("[br_igmp_process_ignore_group_list] Reject rule mask %08x\n", ignoreMsgPtr[inputIndex].mask.s6_addr32[0]);
		}
		else if ((ignoreMsgPtr[inputIndex].address.s6_addr32[0] & htonl(0xF0000000)) != htonl(0xE0000000) ) 
		{
			ret = -EINVAL;
			printk ("[br_igmp_process_ignore_group_list] Reject rule address %08x\n", ignoreMsgPtr[inputIndex].address.s6_addr32[0]);
		}
		else 
		{
			struct br_mcast_control_filter_entry *newFilter = kmem_cache_alloc(br_igmp_mc_fil_cache, GFP_ATOMIC);
			if (newFilter) 
			{
				newFilter->group.s6_addr32[0] = ignoreMsgPtr[inputIndex].address.s6_addr32[0];
				newFilter->mask.s6_addr32[0] = ignoreMsgPtr[inputIndex].mask.s6_addr32[0];
				hlist_add_head(&newFilter->node, &multiConfig.igmp_control_filter_list);
			}
			else 
			{
				ret = -ENOMEM;
				printk ("[br_igmp_process_ignore_group_list] Reject memory\n");
			}
		}
	}
	spin_unlock_bh(&multiConfig.cfgLock);
...
if(!list_empty(&igmp_fdb->rep_list)){
	                         ...
	                         rep_entry_n, &igmp_fdb->rep_list, list) 
	{
		    ...
		    (repMac && (0 == memcmp(rep_entry->repMac, repMac, ETH_ALEN))))
		{
			if ( br->igmp_snooping )
			{
				mcpd_nl_send_igmp_purge_entry(br, igmp_fdb, rep_entry);
			}
                       ...
                       uint32_t info)
{
	...
	if(!SNOOP_IN_IS_ADDR_L2_MCAST(rxGrp) && !br_igmp_control_filter(NULL, rxGrp->s_addr) )
		return 0;
	...
	spin_lock_bh(&br->mcl_lock);
	if (br_mc_fdb_update(br, prt, rxGrp, txGrp, rep, repMac, rep_proto_ver, mode, src, from_dev, info))
	{
		kmem_cache_free(br_igmp_mc_fdb_cache, mc_fdb);
		kmem_cache_free(br_igmp_mc_rep_cache, rep_entry);
		spin_unlock_bh(&br->mcl_lock);
                       ...
                       int is_routed)
{
	...
	if ( ip_fast_csum(pipmcast, pipmcast->ihl) )
	{
		kfree_skb(skb);
		return 1;
	}
	    ...
	    (br->igmp_snooping || is_multicast_switching_mode_host_control()))
	{
		...
		int    len = ntohs(pipmcast->tot_len) - (pipmcast->ihl * 4); 
		__wsum csum = csum_partial(pigmp, len, 0);
		if ( csum_fold(csum) )
		{
			kfree_skb(skb);
			return 1;
...
int __init br_igmp_snooping_init(void)
{
	...
	br_igmp_mc_fil_cache = kmem_cache_create("bridge_igmp_mc_fil_cache",
	                                         sizeof(struct br_mcast_control_filter_entry),
	                                         0,
	                                         SLAB_HWCACHE_ALIGN, NULL);
	if (!br_igmp_mc_fil_cache)
	{
		kmem_cache_destroy(br_igmp_mc_fdb_cache);
	...
	spin_lock_init(&multiConfig.cfgLock);
...
void br_igmp_snooping_fini(void)
{
	kmem_cache_destroy(br_igmp_mc_fdb_cache);
	kmem_cache_destroy(br_igmp_mc_rep_cache);
	kmem_cache_destroy(br_igmp_mc_fil_cache);
