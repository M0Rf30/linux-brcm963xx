...
static void mcpd_nl_process_ignore_group (struct sk_buff *skb)
{
    struct nlmsghdr* nlh                   = (struct nlmsghdr *)skb->data;
    char*  ptr                             = NLMSG_DATA(nlh);
    t_MCPD_IGNORE_GROUP_MESSAGE* ignoreMsg = (t_MCPD_IGNORE_GROUP_MESSAGE*)( ptr + sizeof(sizeof(t_MCPD_MSG_HDR)));

    if (IPPROTO_IGMP == ignoreMsg->proto) 
    {
      (void) br_igmp_process_ignore_group_list (ignoreMsg->count, ignoreMsg->ignoreEntry);
    }
#if defined(CONFIG_BCM_KF_MLD) && defined(CONFIG_BR_MLD_SNOOP)
    else if (IPPROTO_ICMP == ignoreMsg->proto) 
    {
      (void) br_mld_process_ignore_group_list (ignoreMsg->count, ignoreMsg->ignoreEntry);
    }
#endif
}
...
void mcpd_nl_send_igmp_purge_entry(struct net_bridge *br,
                                   struct net_bridge_mc_fdb_entry *igmp_entry, 
                                   struct net_bridge_mc_rep_entry *rep_entry)
{
    ...
    purge_entry->parent_ifi    = br->dev->ifindex;
    purge_entry->rep_ifi       = igmp_entry->dst->dev->ifindex;
